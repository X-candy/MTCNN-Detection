...
void MTCNN::GenerateBBox(Blob<float>* confidence, Blob<float>* reg_box,
    ...
    for (int i = 0; i<spatical_size; i++) {
        if (confidence_data[i] >= thresh) {
            ...
            faceBox.xmin = (float)(x * __PNET_S__) / scale;
            faceBox.ymin = (float)(y * __PNET_S__) / scale;
            faceBox.xmax = (float)(x * __PNET_S__ + __PNET_CELL_SIZE__ - 1.f) / scale;
            faceBox.ymax = (float)(y * __PNET_S__ + __PNET_CELL_SIZE__ - 1.f) / scale;
...
MTCNN::MTCNN(const string& proto_model_dir) {
    ...
    PNet_.reset(new Net<float>((proto_model_dir + "/PNet.prototxt"), TEST));
    PNet_->CopyTrainedLayersFrom(proto_model_dir + "/PNet.caffemodel");
    RNet_.reset(new Net<float>((proto_model_dir + "/RNet.prototxt"), TEST));
    RNet_->CopyTrainedLayersFrom(proto_model_dir + "/RNet.caffemodel");
    ONet_.reset(new Net<float>((proto_model_dir + "/ONet.prototxt"), TEST));
...
vector<FaceInfo> MTCNN::ProposalNet(const cv::Mat& img, int minSize, float threshold, float factor) {
    ...
    for (int i = 0; i < scales.size(); i++) {
        ...
        for (int k = 0; k < spatial_size; ++k) {
            input_data[k] = float((img_data[k][0] - __MEAN_VAL__)* __STD_VAL__);
            input_data[k + spatial_size] = float((img_data[k][1] - __MEAN_VAL__) * __STD_VAL__);
            input_data[k + 2 * spatial_size] = float((img_data[k][2] - __MEAN_VAL__) * __STD_VAL__);
        }
        PNet_->Forward();
...
vector<FaceInfo> MTCNN::NextStage(const cv::Mat& image, vector<FaceInfo> &pre_stage_res, int input_w, int input_h, int stage_num, const float threshold) {
    ...
    for (int n = 0; n < batch_size; ++n) {
        ...
        Mat roi = image(cv::Rect(cv::Point((int)box.xmin, (int)box.ymin), cv::Point((int)box.xmax, (int)box.ymax))).clone();
        ...
        for (int k = 0; k < spatial_size; ++k) {
            input_data_n[k] = float((roi_data[k][0] - __MEAN_VAL__)*__STD_VAL__);
            input_data_n[k + spatial_size] = float((roi_data[k][1] - __MEAN_VAL__)*__STD_VAL__);
            input_data_n[k + 2 * spatial_size] = float((roi_data[k][2] - __MEAN_VAL__)*__STD_VAL__);
        }
    }
    switch (stage_num) {
        case 2: {
...
std::vector<FaceInfo> MTCNN::Detect(const cv::Mat& image, const int minSize, const float* threshold, const float factor, const int stage) {
    ...
    if (stage >= 2 && pnet_res.size()>0){
        if (__PNET_MAX_DET__ < (int)pnet_res.size()){
            pnet_res.resize(__PNET_MAX_DET__);
        }
        int num = (int)pnet_res.size();
        int size = (int)ceil(1.f*num / __MINBATCH__);
        for (int iter = 0; iter < size; ++iter){
            int start = iter*__MINBATCH__;
            int end = min(start + __MINBATCH__, num);
